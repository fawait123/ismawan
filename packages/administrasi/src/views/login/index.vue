...
import { ResponseMessage } from '@/helpers/index'
...
const { validate, isValid, getError, scrolltoError } = useValidation(loginSchema, loginRef, {
    mode: 'lazy',
});
...
const handleLogin = async () => {
    try {
        await validate()
        if (isValid.value) {
            const loginRequest = await doRequest({
                url: 'auth',
                method: "post",
                data: loginRef.value
            })
        ...
        <div class="flex flex-col items-center justify-center">
            <div
                ...
                <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                    ...
                    <form>
                        <CustomInputGroup placeholder="Masukan Username" label="Username" v-model="loginRef.username"
                            :invalid="!!getError('username')" :error-message="getError('username')" class-name="mb-8" />

                        <CustomInputGroup placeholder="Masukan Password" label="Password" type="password"
                            v-model="loginRef.password" :invalid="!!getError('password')"
                            :error-message="getError('password')" class-name="mb-8" />

                        <div class="flex items-center justify-between mt-2 mb-8 gap-8">
                            <div class="flex items-center">
                                <Checkbox v-model="loginRef.rememberMe" id="rememberme1" binary class="mr-2"></Checkbox>
                                <label for="rememberme1">Remember me</label>
                            </div>
                            <span class="font-medium no-underline ml-2 text-right cursor-pointer text-primary">Forgot
                                password?</span>
                        </div>
                        <Button type="button" label="Sign In" class="w-full" @click="handleLogin"></Button>
                    </form>
                </div>
            </div>
        </div>
