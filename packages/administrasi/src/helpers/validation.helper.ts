...
export default function <T extends ZodTypeAny>(schema: T, data: MaybeRefOrGetter<Record<string, unknown>>, options?: { mode: 'eager' | 'lazy' }) {
    ...
    const getErros = (keys: string, path: string, index: number) => {
        const errorKey = `${keys},${index},${path}`
        return get(errors.value, `${errorKey.replaceAll('.', ',')}.0.message`)
    }
    ...
    return { validate, errors, isValid, clearErrors, getErros, getError, scrolltoError }
}
